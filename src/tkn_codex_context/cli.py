...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="tkn-codex-context",
        description="Capture all local Codex conversations and refresh their knowledge pipeline.",
    )
    ...
    for name, help_text in (
        ...
        ("pull", "Capture new/changed logs and resume unfinished stages across every scope"),
    ):
    ...
    scopes = commands.add_parser("scopes", help="Inspect Project and explicitly configured work scopes")
    scopes.add_subparsers(dest="scopes_command", required=True).add_parser("list")
    ...
    ingest = raw.add_subparsers(dest="raw_command", required=True).add_parser(
        "ingest", help="Write immutable captures (writes by default)"
    )
    ...
    for name, dest, label in (
        ("thread-notes", "notes_command", "Thread Notes"),
        ("decisions", "decisions_command", "Decision Records"),
        ("working-context", "working_context_command", "Working Context"),
    ):
        ...
        if name == "thread-notes":
            ...
            build.add_argument("--scope", help="Exact scope ID from scopes list; defaults to every scope")
...
def _progress(value: dict[str, Any]) -> None:
    ...
    if event_type == "thread-start":
        ...
        LOGGER.info(
            "Starting Working Context synthesis for scope %s: %s sources",
        ...
        log_success(
            LOGGER,
            "Completed Working Context synthesis for scope %s%s — Working Context: %s",
            value.get("projectId", "unknown"),
            _metric_summary(value),
        ...
        LOGGER.error(
            "Failed Working Context synthesis for scope %s: %s",
            value.get("projectId", "unknown"),
            value.get("error", "unknown error"),
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "scopes":
            _emit(read_json(config.data_root / "catalog" / "scopes.json", {"scopes": []}))
            return 0
        if getattr(args, "artifact", None) is not None:
            validators = {
                "thread-notes": validate_thread_note,
                "decisions": validate_decision_record,
                "working-context": validate_working_context,
            }
        ...
        LOGGER.info("Thread states: %s; scope states: %s", report["threadCounts"], report["scopeCounts"])
        for warning in report.get("warnings", []):
            LOGGER.warning("%s", warning)
        _emit(
            report
            if args.full_output
            else {key: value for key, value in report.items() if key not in {"threads", "scopeResults", "rawIngest"}}
        )
        failed = bool(report["failed"] or report["threadCounts"].get("failed") or report["scopeCounts"].get("failed"))
        if failed:
