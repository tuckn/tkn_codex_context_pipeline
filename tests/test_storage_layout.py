...
from tkn_genai_chat_note.config import AppConfig, write_config
...
def test_migration_rejects_corrupt_raw_and_wrong_source(tmp_path: Path) -> None:
    config, source_config, source = legacy_store(tmp_path)
    config.chat.providers.codex.sources = {"other": config.source_settings}
    ...
    config.chat.providers.codex.sources = {"windows": config.source_settings}
...
def test_two_source_stores_are_independent_and_cannot_reuse_a_root(tmp_path: Path) -> None:
    ...
    first.chat.providers.codex.sources = {"pc-one": first.source_settings}
    ...
    second.chat.providers.codex.sources = {"pc-two": second.source_settings}
...
def test_explicit_paths_are_final_and_defaults_are_source_scoped(
    ...
    config = AppConfig()
    assert config.data_root == tmp_path / ".tkn/genai_chat_note_pipeline/data/codex/windows"
...
def test_migrated_legacy_locators_resolve_inside_copied_store(tmp_path: Path) -> None:
    from tkn_genai_chat_note.references import resolve_store_ref
...
def test_storage_roots_cannot_overlap_a_disabled_provider(tmp_path: Path) -> None:
    config = config_for(tmp_path)
    config.chat.providers.claude_code.sources["windows-claude-code"].data_root = config.raw_root
